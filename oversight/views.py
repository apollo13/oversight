    ...
    for sensor in sensors:
        sensor_data = (
            LogEntry.objects.filter(
                sensor=sensor, datetime__gt=now() - timedelta(days=2)
            )
            .order_by("datetime")
...
def sensor_detail(request, slug):
    ...
    if request.method == "POST" and request.user.is_authenticated():
        ...
        if export_form.is_valid():
            ...
            sensor_data = (
                sensor_data.filter(datetime__gte=dt)
                .values_list("datetime", "value")
                .iterator()
            )
...
def sensor_compare(request):
    ...
    sensors = (
        Sensor.objects.filter(api_endpoint__in=sensors)
        .select_related("current_log")
        .order_by("name")
    )
